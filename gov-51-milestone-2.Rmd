...
found most of this chunk unnecessary:
  - don't need to rename independenth and other
  - also don't need to repeat all the steps with a subset that has no NAs; can just drop those as needed
...
table(survey$political_party, survey$length_in_this_facility)
prop.table(table(survey$political_party, survey$length_in_this_facility))
...
table(survey_no_NA$political_party, survey_no_NA$length_in_this_facility)
prop.table(table(survey_no_NA$political_party, survey_no_NA$length_in_this_facility))
```

## Assault Weapons Ban

instead of turning it into a binary variable, made it a factor variable
  - could also assign weights instead of strict {0,1} (?)
can use ifelse to clean up creation of binary
...
table(survey$assault_weapon_ban_stance, survey$length_in_this_facility)
prop.table(table(survey$assault_weapon_ban_stance, survey$length_in_this_facility))
...
table(survey_no_NA$assault_weapon_ban_stance, survey_no_NA$length_in_this_facility)
prop.table(table(survey_no_NA$assault_weapon_ban_stance, survey_no_NA$length_in_this_facility))
...
table(survey$marijuana_legalization_stance, survey$length_in_this_facility)
prop.table(table(survey$marijuana_legalization_stance, survey$length_in_this_facility))
...
table(survey_no_NA$marijuana_legalization_stance, survey_no_NA$length_in_this_facility)
prop.table(table(survey_no_NA$marijuana_legalization_stance, survey_no_NA$length_in_this_facility))
```
...
table(survey$border_security_stance, survey$length_in_this_facility)
prop.table(table(survey$border_security_stance, survey$length_in_this_facility))
...
table(survey_no_NA$border_security_stance, survey_no_NA$length_in_this_facility)
prop.table(table(survey_no_NA$border_security_stance, survey_no_NA$length_in_this_facility))
```
...
table(survey$minimum_wage_stance, survey$length_in_this_facility)
prop.table(table(survey$minimum_wage_stance, survey$length_in_this_facility))
...
table(survey_no_NA$minimum_wage_stance, survey_no_NA$length_in_this_facility)
prop.table(table(survey_no_NA$minimum_wage_stance, survey_no_NA$length_in_this_facility))
```
...
table(survey_no_NA$mean_four_political_views, survey_no_NA$length_in_this_facility)
prop.table(table(survey_no_NA$mean_four_political_views, survey_no_NA$length_in_this_facility))
```
